async function getShopifyProductsWithVariants(postgres, req, res) {
    try {
        const query = `
        SELECT 
        ...
        console.error('Error executing query getShopifyProductsWithVariants:', error);
        // Send error response back to the client
        res.status(500).json({ error: 'An error occurred while processing your request' });
    }
}

module.exports = getShopifyProductsWithVariants;
